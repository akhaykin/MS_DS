...
1.  Describe the distribution of responses in this sample. The distribution for Samp1 is 74% "Benefit" rather then 80% Benefits from the entire data frame. And 26% Benefits rather than 20% as int he entire data frame population. How does it compare to the distribution of responses in the population. It is very similar. **Hint:** Although the `sample_n` function takes a random sample of observations (i.e. rows) from the dataset, you can still refer to the variables in the dataset with the same names. Code you presented earlier for visualizing and summarizing the population data will still be useful for the sample, however be careful to not label your proportion `p` since you're now calculating a sample statistic, not a population parameters. You can customize the label of the statistics to indicate that it comes from the sample.
...
2.  Would you expect the sample proportion to match the sample proportion of another student's sample? Yes.. Why, or why not? Becaseu we are all setting the same seed. If the answer is no, would you expect the proportions to be somewhat different or very different? Ask a student team to confirm your answer.
...
3.  Take a second sample, also of size 50, and call it `samp2`. How does the sample proportion of `samp2` compare with that of `samp1`? Very similar. Suppose we took two more samples, one of size 100 and one of size 1000. Which would you think would provide a more accurate estimate of the population proportion? The same of 1000 would be more accurate. 
...
4.  How many elements are there in `sample_props50`? 15,000 Describe the sampling distribution, and be sure to specifically note its center. Normal distribution, with an average of 20%. Make sure to include a plot of the distribution in your answer.
...
5. To make sure you understand how sampling distributions are built, and exactly what the `rep_sample_n` function does, try modifying the code to create a sampling distribution of **25 sample proportions** from **samples of size 10**, and put them in a data frame named `sample_props_small`. Print the output. How many observations are there in this object called `sample_props_small? 25*10 = 250 observations. What does each observation represent? Each observation represents the proportion of (Doesn't benefit) from a sample of 10.
...
6.  Use the app below to create sampling distributions of proportions of *Doesn't benefit* from samples of size 10, 50, and 100. Use 5,000 simulations. What does each observation in the sampling distribution represent? Each observation represents proportion of (doesn't benefit) from a sample size 10, 50, and 100 ussing 5000 observations. How does the mean, standard error, and shape of the sampling distribution change as the sample size increases? The mean changes closer and closer to the mean of 15000 as the sample size get bigger.As the sample size grows the SE gets smaller  and smaller. How (if at all) do these values change if you increase the number of simulations? Sample size changes the SE, NOT the amount of replicates. (You do not need to include plots in your answer.)
