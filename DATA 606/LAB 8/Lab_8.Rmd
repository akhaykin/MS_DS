...
2.  What type of plot would you use to display the relationship between the personal
    freedom score, `pf_score`, and one of the other numerical variables? Dot Plot. Plot 
    this relationship using the variable `pf_expression_control` as the predictor.
    
```{r dot_plot_expr_vs_pf_score}
ggplot(hfi, aes(x=pf_expression_control, y=pf_score)) + 
  geom_point()
  
```
    
    Does the relationship look linear? Yes. If you knew a country's `pf_expression_control`,
    ...
    predict the personal freedom score? Yes, the relationship exists.
...
3.  Looking at your plot from the previous exercise, describe the relationship 
    between these two variables.The relationship is linear, and positive. As pf_expression moves up so does pf_score. There are some outliar observations that stray from the line fit.   Make sure to discuss the form, direction, and 
...
4.  Using `plot_ss`, choose a line that does a good job of minimizing the sum of
    ...
    squares that you got? The was  1031.364 How does it compare to your neighbors?
...
5.  Fit a new model that uses `pf_expression_control` to predict `hf_score`, or
    ...
    pressure on media content? For ever increase of political pressure on media content units, the amount of human freedom units move up by 0.349862.
...
6.  If someone saw the least squares regression line and not the actual data, how
    ...
    for `pf_expression_control`? (6.7*.49143) + 4.61707 = 7.909651 fitted y value.
    
    Is this an overestimate or an underestimate, and
    by how much? Neither, it ovber estimates for some and overestimates for other points. In other words, what is the residual for this prediction?
    
```{r score_pof_expression}
hfi %>%
  drop_na(pf_score, pf_expression_control) %>%
  broom::augment(m1, .) %>%
  select(pf_score, pf_expression_control, .fitted, .resid) %>%
  filter(between(pf_expression_control, 6.65, 6.75)) %>%
  View()
```
...
7.  Is there any apparent pattern in the residuals plot? No, there does not seem to be a pattern. What does this indicate
    about the linearity of the relationship between the two variables? There is linear relationship.
...
8.  Based on the histogram and the normal probability plot, does the nearly 
    normal residuals condition appear to be met? Yes.
...
9.  Based on the residuals vs. fitted plot, does the constant variability condition 
    appear to be met? Yes there is the appearance of constant variability.
...
-   Choose another freedom variable and a variable you think would strongly
    ...
    linear model. At a glance, does there seem to be a linear relationship? Yes there is definitely a linear relationship between personal religious freedom and personal religious restrictions freedom.
    
```{r pf_religion_pf_religion_res}
ggplot(hfi, aes(pf_religion, pf_religion_restrictions)) + geom_point() + stat_smooth(formula = "y~x", method = "lm", se = FALSE)

```
    
-   How does this relationship compare to the relationship between 
    `pf_expression_control` and `pf_score`? m1 has a higher $R^2$. Use the $R^2$ values from the two 
    model summaries to compare. Does your independent variable seem to predict
    your dependent one better? No, the independent variable in m1 predicts better than m3.  Why or why not? Because the $R^2$ value is greater, and closer to 1.0.

```{r m3}
m3 <- lm(pf_religion ~ pf_religion_restrictions, data = hfi)
