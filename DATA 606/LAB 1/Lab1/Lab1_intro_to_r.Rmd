...
2. Is there an apparent trend in the number of girls baptized over the years? 
   ...
   interpretation.) The number of girls baptized is increasing over the years. Therefore there must be a positive linear relationship.
...
3. Now, generate a plot of the proportion of boys born over time. What do you see? The number of boys baptized increased from 1660, then it became a positive linear relationship.
...
4.  What years are included in this data set?  1629 to 1710
```{r question1}
min(arbuthnot$year)
max(arbuthnot$year)
```
...
4. What are the dimensions of the 
    data frame? 82 rows and 6 columns  
```{r }
dim(arbuthnot)
```
    
4. What are the variable (column) names? year, boys, girls
```{r }
colnames(arbuthnot)
```

5.  How do these counts compare to Arbuthnot's? Are they of a similar magnitude? No they are different than arbuthnot's, there are millions and ranges from 1940 to 2002 and the arbuthnots in thousands from 1969 to 1710.

```{r}
arbuthnot$girls + arbuthnot$boys
```

6.  Make a plot that displays the proportion of boys born over time. What do you see? The proportions of boys had increased in 1960s, and then decreased in late 1970s, and then increased again in 1980s. Does Arbuthnot's observation about boys being born in greater proportion than girls hold up in the U.S.? Yes, Arbutnoths observations about boys being born in greater proportions than girls is correst in US true. Include the plot in your response. *Hint:* You should be able to reuse your code from Exercise 3 above, just replace the dataframe name.
...
7.  In what year did we see the most total number of births in the U.S.? The year with most birth was 1705. *Hint:*
