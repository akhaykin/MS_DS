...
1.  Make a plot (or plots) to visualize the distributions of the amount of calories from fat of the options from these two restaurants.  How do their centers, shapes, and spreads compare? The centers are very similar, shapes are very similar. However, the spread for McDonalds has three restaurants with CAL FROM FAT above approximately 750.

```{r}
ggplot(dairy_queen, mapping = aes(cal_fat)) + geom_histogram()

ggplot(mcdonalds, mapping = aes(cal_fat)) + geom_histogram()

bind_rows(mcdonalds, dairy_queen) %>%
  ggplot(aes(cal_fat)) + geom_histogram() + facet_wrap(~restaurant)
```
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    distribution? Dairy Queen restaurant has a close to normal distribution, while McDonald has more left skewed.
...
3.  Make a normal probability plot of `sim_norm`.  Do all of the points fall on the line? No they do not. How does this plot compare to the probability plot for the real data? The simulation is more closely resembling normal distribution. (Since `sim_norm` is not a data frame, it can be put directly into the `sample` argument and the `data` argument can be dropped.)
...
4.  Does the normal probability plot for the calories from fat look similar to the plots 
    created for the simulated data? They look very similar, with the exception of the curve at the top for the original data.   That is, do the plots provide evidence that the
    calories are nearly normal? They are.
```{r}
...
5.  Using the same technique, determine whether or not the calories from McDonald's
    menu appear to come from a normal distribution. The original data is skewed, while simulations are close to normal.
...
These two probabilities are not similar, therefore I would say no for calories are not normal.

```{r}
pnorm(q = 600, mean =  dqmean, sd = dqmean)
```
...
```{r messege=TRUE, warning=FALSE, page.print=TRUE}
