...
1.   What are the counts within each category for the amount of days these students have texted while driving within the past 30 days? please see below.

```{r text_while_driving_30d}
...
2.  What is the proportion of people who have texted while driving every day in the past 30 days and never wear helmets? 3.4% responded text while driving and never wearing helmets.

```{r text_and_no_helmet}

yrbss %>% count(text_while_driving_30d, helmet_12m) %>%
  mutate(prop=n/sum(n)) %>%
...
```{r indicator-texting}
no_helmet <- no_helmet %>%
  mutate(text_ind = ifelse(text_while_driving_30d %in% "30", "yes", "no"))
```
...
```{r}
(.07209-median(c(.0597, .07209)))/median(c(.0597, .07209))
```
    
    

4.  Using the `infer` package, calculate confidence intervals for two other 
    categorical variables (you'll need to decide which level to call "suc_gradecess", 
    ...
    countries first, and then use these data sets to construct the confidence intervals. For female and grade 12 the upper and lower ci are 0.256    0.276.
    
```{r gender_female_count}
yrbss %>% count(gender)%>%
...
```{r female_grade_12_only}
yrbss %>% count(gender, grade)%>%
...
female_12 <- yrbss %>%
  filter(gender=="female") %>%
  mutate(grade_12 = ifelse(grade %in% "12", "yes", "no"))
```

```{r female_grade_12_ci}
female_12 %>%
  specify(response = grade_12, success = "yes") %>% 
  generate(reps = 1000, type = "bootstrap") %>%
  calculate(stat = "prop") %>%
  get_ci(level = 0.95)
```
...
```{r}
(.276-median(c(.256, .276)))/median(c(.266, .276))
```
...
5.  Describe the relationship between `p` and `me`. Include the margin of
    ...
    a given sample size, for which value of `p` is margin of error maximized?.50
...
Normal distribution, center is .1, spread is .05, shape is normal.
...
7.  Keep $n$ constant and change $p$. How does the shape, center, and spread 
    ...
    min and max for the $x$-axis for a better view of the distribution. As the p is changed the data shifts so the center is at p. if we change n the data seems to be skewed to right or left.
    
8.  Now also change $n$. How does $n$ appear to affect the distribution of $\hat{p}$?
As the population is changed the shape stays relatively the same.
