...
1.  Is this an observational study or an experiment? This is an observational study. The original research question posed in the paper is whether beauty leads directly to the differences in course evaluations. Given the study design, is it possible to answer this question as it is phrased? The variables are merely correlated  If not, rephrase the question. Because causality could not be established.

2.  Describe the distribution of `score`. Is the distribution skewed? The distribution is slightly right skewed. What does that tell you about how students rate courses? More students score a higher score than lower.  Is this what you expected to see? No. Why, or why not? I expected more evenly distributed scores, but not as many students rate lower.
    
```{r}
ggplot(data = evals, aes(x = score)) + geom_histogram()

ggplot(data = evals, aes(sample = score)) +
  stat_qq() +
  stat_qq_line()
```
...
Before you draw conclusions about the trend, compare the number of observations in the data frame with the approximate number of points on the scatterplot. Is anything awry? NO I cannot see anything wrong.

4.  Replot the scatterplot, but this time use `geom_jitter` as your layer. What 
    was misleading about the initial scatterplot? Original scatterplot had overlapping data, and more data.
...
5.  Let's see if the apparent trend in the plot is something more than
    ...
    predictor? Does it appear to be a practically significant predictor? Yes it is significant becasue the P value is less than .05.  
...
6.  Use residual plots to evaluate whether the conditions of least squares
    ...
    the Simple Regression Lab for a reminder of how to make these).The data shows a positive linear relationship. The residuals show a left skewed distribution. Normal probability plot of residuals point do not follow the upper Q. Residual plot observation have constant variability.
...
17. The original paper describes how these data were gathered by taking a
    ...
    of linear regression? The class courses are independent of each other so it should bot affect the evaluation scores can be affected,

18. Based on your final model, describe the characteristics of a professor and 
    course at University of Texas at Austin that would be associated with a high
    evaluation score. Professor would be younger male teaching one credit class, he would not belong to a minority group, he would have received this degree from a university where english is the language. The professor would habe a black and white picture and who have been rated beautiful.

19. Would you be comfortable generalizing your conclusions to apply to professors
    generally (at any university)?  No. Why or why not? The sample size of 6 is       too small. Also some of the predictor variables are subjective and may vary       with culture, Beauty, for one, is in the eye of the beholder. Picture             preference may also be culturally biased
